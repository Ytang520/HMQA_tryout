...
class AGCN_part(layers.Layer):
    ...
    def __init__(self, out_channels, A, coff_embedding=12):
        super(AGCN_part, self).__init__()

        # 可以输入不同子层的不同A_k，此处暂时不考虑；目前假设只有一张图，即A
        # A.shape = N*N 表示节点间关系

        self.embedding_size = math.ceil(out_channels/4)  # 设置为 channel*100(T的大小)/4 的结果
        # self.num_parts = num_parts

        # AGCN_part 中使用的 embedding 的值
        self.out_channels = out_channels  # out_channels 表示每一个 block 输出的维度（可能有多个block）
        # self.inter_c = out_channels // coff_embedding  # 向下取整, inter_channels 为每个embedding的C_e —— 由输出决定
        self.A = tf.transpose(A)
        self.B = tf.Variable(tf.random.normal(shape=A.shape), dtype=tf.float32, name='matrix_B')  # 这里表示和A的shape一样, __init__ 中默认可训练

        # AGCN_part 中使用的实例层
        # 在使用 TensorFlow 中的 Conv2D 类时，输入张量的维度通常为 (batch_size, height, width, channels)
        self.Conv2D_theta = layers.Conv2D(
        ...
        # 加速计算：Batchnorm和relu
        self.Batchnorm = layers.BatchNormalization()  # 对每一个通道都进行batchnormalization，计算每个通道的均值和方差，加速网络; 这里不必加入任何参数;
        # layers.Flatten() 多维展开成一维
        self.relu = layers.LeakyReLU()  # 对每一个元素都进行 ReLU 操作，让输出更稳定
        # layers.ELU

    def call(self, inputs_per_part):
        # embedding 输入向量和计算矩阵C
        C_theta = self.Conv2D_theta(inputs_per_part)
        C_fai = self.Conv2D_fai(inputs_per_part)

        # 此处 C_theta 和 C_fai shape = [None, T, N, C_e]
        # 转变为：shape = [None, C_e, T, N],方便后续运算
        C_theta = tf.transpose(C_theta, [0, 3, 1, 2])  # 考虑到卷积最后一个维度为channels，这里做转置
        C_fai = tf.transpose(C_fai, [0, 3, 1, 2])  # 考虑到卷积最后一个维度为channels，这里做转置

        C_theta = tf.transpose(tf.reshape(C_theta, [-1, C_theta.shape[-3] * C_theta.shape[-2], C_theta.shape[-1]]), [0, 2, 1])
        # print(C_theta.shape)
        C_fai = tf.reshape(C_fai, [-1, C_fai.shape[-3] * C_fai.shape[-2], C_fai.shape[-1]])  # 此处第一个输入为 None 表示 batch_size
        C_multiple = tf.einsum('imj,ijk->imk', C_theta, C_fai)
        C = tf.nn.softmax(C_multiple, axis=-1)  # 对每一行处理softmax, 得到C矩阵, 此处C矩阵维度:shape = [None, N, N]

        # 为之后矩阵乘法做准备，使得满足message passing条件
        C = tf.transpose(C, [0, 2, 1])

        # 加和矩阵A, B和C
        attention_graph = C + self.A + self.B
        inputs_per_part_raw = tf.transpose(inputs_per_part, [0, 3, 1, 2])  # shape=(None, C_e, T, N)
        output_middle = tf.einsum('ijkm,iml->ijkl', inputs_per_part_raw, attention_graph)  # 这里为四维，原因在于None：batchsize
        # 此处output_middle为: shape=(None, C_e, T, N)

        output_middle = tf.transpose(output_middle, [0, 2, 3, 1])  # shape = [None, T, N, C_e]
